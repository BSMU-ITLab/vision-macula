...
class MaskDrawMode(Enum):
    ...
    # Описание вынесено в словарь
    description_map = {
        REDRAW_ALL: "Completely replace the existing mask with the new mask.",
        OVERLAY_FOREGROUND: "Apply the new mask only where its own pixels are equal to foreground value, "
                            "preserving the existing mask elsewhere.",
        FILL_BACKGROUND: "Apply the new mask only on the background pixels of the existing mask, "
                         "leaving other pixels unchanged."
    }

    @property
    def description(self) -> str:
        return self.description_map[self]
