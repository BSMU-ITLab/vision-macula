...
class FoveaMaskLoaderPlugin(Plugin):
    ...
    def _active_layered_image(self) -> LayeredImage | None:
        """Возвращает LayeredImage активного окна или None (с сообщением)."""
    ...
    def _select_fovea_mask_file(self):
        # Активное окно получаем ДО диалога, иначе теряется фокус.
    ...
    def _add_fovea_mask_layer(self, layered_image: LayeredImage, mask_fovea: np.ndarray):
