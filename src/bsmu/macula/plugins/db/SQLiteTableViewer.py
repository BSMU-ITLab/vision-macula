...
from bsmu.macula.plugins.db.constants import (
    DROPDOWN_DB_VALUES, DROPDOWN_DISPLAY_MAP, BLOCKS,
    COLUMN_INDEX_TO_FIELD_NAME, COLUMNS_EYES, EYE_TABLE_NAME,
    ERROR_TEXT, PACIENTS_TABLE_NAME, PARAMETERS_PATIENTS_SELECT,
)
...
from PySide6.QtWidgets import (
    QWidget, QTableView, QVBoxLayout, QHBoxLayout, QPushButton,
    QScrollArea, QGroupBox, QLineEdit, QTabWidget, QMessageBox,
...
class TableWidgetExample(QWidget):
    ...
    def __init__(self, bd_path, dataPath):
        ...
        self.db_manager = DatabaseManager()      # ничего не открывает
        self.db = None
        self._db_loaded = False
        self._db_load_failed = False

        self.query_builder_eye = None
        self.query_builder_patient = None

        # атрибуты, которые раньше появлялись только после клика
        self.patient_id_clicked = None
        self.appointment_id = None
        self.age = None

        self.layoutBoxes = QVBoxLayout()

        # --- UI без обращения к БД ---
        self._setup_ui()
    ...
    def ensure_db_loaded(self) -> bool:
        ...
        self._db_loaded = True  # <-- ДО загрузки данных
        self.create_forder()
    ...
    def showEvent(self, event):
        ...
        # первый показ виджета = момент, когда реально нужна БД
    ...
    def _setup_patients_table(self):
        """Настройка таблицы пациентов (без запроса к БД)."""
        ...
        self.appointment_edit_delegate = EditPacientDelegate(
            ':/dbicons/edit.png', self.patients_table, self._open_dialog,
        )
        self.patients_table.setItemDelegateForColumn(
            self.patients_model.columnCount() - 1, self.appointment_edit_delegate,
        )
    ...
    def _load_patients_data(self):
        # Без проверки ensure_db_loaded — иначе бесконечная рекурсия.
    ...
    def _load_appointments(self, patient_id: int):
        ...
        if not query.exec(
            f"SELECT id, date, duration_of_the_disease "
            f"FROM appointments WHERE pacient_id = {patient_id}"
        ):
    ...
    def _load_appointment_data(self, appointment_id: int):
        ...
        column_clause = ", ".join(
            [f"eyes.{col}" if not col.startswith("injections.") else col for col in columns]
        )
    ...
    def get_value_by_field(self, model, row_index, field_name):
        ...
        return model.data(model.index(row_index, column_index))
    ...
    def _update_ui(self, eye_id_field):
        ...
        if eye_id_field is not None:
            for row in range(self.appointment_data_model.rowCount()):
                index = self.appointment_data_model.index(row, 0)
                id_value = self.appointment_data_model.data(index)
                eye_in = self.appointment_data_model.data(
                    self.appointment_data_model.index(row, 1)
                )
                t = None
                if eye_in in ('L', '1', 'ос'):
                    t = 1
                ...
                self.tab_widget.addTab(
                    self._create_eye_tab2(t, id_value, rowInt),
                    "Правый глаз" if t == 0 else "Левый глаз",
                )
                rowInt += 1

        folder = os.path.join(
            self.base_dir, str(self.patient_id_clicked), str(self.appointment_id)
        )
    ...
    def _create_eye_tab2(self, eye_index: int = None,
                         eye_id_field: int = None, rowInt: int = None) -> QScrollArea:
        ...
        self.formDict[eye_index] = DynamicFormBuilder(
            BLOCKS, new_data, eye_index,
            self.appointment_data_model,
            self.db_manager.get_connection(),
            self.patient_id_clicked, rowInt,
        )
        ...
        if eye_id_field is None:
            ...
            self.edit_toggle_button.clicked.connect(
                lambda: self._toggle_edit_mode_2(self.edit_toggle_button, eye_index)
            )
    ...
    def collect_form_data(self, form_dict, eye_index, blocks, get_column_name_fn):
        all_fields = []
        all_values = {}
    ...
    def _toggle_edit_mode_2(self, button: QPushButton, eye_index: int):
        ...
        all_fields, all_values = self.collect_form_data(
            self.formDict, eye_index, BLOCKS, self._get_column_name,
        )
        ...
        if self.appointment_id is None or self.appointment_id == 0:
            ...
            query1.bindValue(":duration_of_the_disease2",
                             all_values.get("duration_of_the_disease"))
            if not query1.exec_():
    ...
    def _open_dialog(self, patient_id: int):
        ...
        if dialog.exec():
            self.patients_model.setQuery(
                "SELECT id, name, sex, year_of_birthday FROM pacients", self.db,
            )
        self._load_patients_data()
    ...
    def _new_create_tabs(self, date):
        ...
        for i in date['e']:
            self.tab_widget.addTab(
                self._create_eye_tab2(0 if i == 'R' else 1),
                "Правый глаз" if i == 'R' else "Левый глаз",
            )
    ...
    def create_forder_for_app(self, appointment_id):
        folder = os.path.join(
            self.base_dir, str(self.patient_id_clicked), str(appointment_id)
        )
        os.makedirs(folder, exist_ok=True)
        print(f"Папка создана: {folder}")
